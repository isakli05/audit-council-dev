...
- Priority/status: P1 / BLOCKED (named blocker updated 2026-09-11 by the REV.6 design-of-record rebind and Option-R1 execution-authority publication: FIRST_PROVEN_QUALIFIED_INSTALLED_PREDECESSOR NOT YET ESTABLISHED / OPTION-R1 EXECUTION AUTHORITY GRANTED BUT CAMPAIGN NOT YET INSTANTIATED OR EXECUTED; the canonical bootstrap-root design-of-record is REV.6 — superseding REV.2 for future campaign mechanics only (REV.2 immutable historical evidence; REV.3-REV.5 immutable superseded intermediates) — and the one-time bootstrap-root policy exception is ADOPTED / AVAILABLE / UNCONSUMED with `BOOTSTRAP_ROOT_CAMPAIGN_AUTHORITY = GRANTED / UNCONSUMED` (operator Option-R1 execution authority; NOT yet consumed by campaign instantiation or auditor execution; granting it does NOT consume the policy exception) and `BOOTSTRAP_ROOT_EVENT_ID = NONE` (`CAMPAIGN_INSTANTIATION = NOT_STARTED`; `MODEL_ENGAGEMENTS_USED = 0`; `AUDITOR_A_EXECUTION = NOT_STARTED`; `AUDITOR_B_EXECUTION = NOT_STARTED`; the two authorized external blind first passes have NOT run; campaign authority is NOT qualification authority and does NOT grant installation); unblock next action = AUTHORIZED OPTION-R1 CAMPAIGN INSTANTIATION + RECORDED EVENT ID + ZERO-MODEL PRELAUNCH/FREEZE GATES; see dated record 33 below; the standing goal is unchanged: reconcile existing evidence before proposing new qualification).
